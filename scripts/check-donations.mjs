...
async function checkDonations() {
  ...
  try {
    // Get all donations
    const { data: donations, error, status } = await supabase
      .from('donations')
      .select('*')
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        console.log('\n⚠️  The "donations" table does not exist in your Supabase database.');
    ...
    if (!donations || donations.length === 0) {
      ...
      console.log('1. The $10 test donation was not saved');
      console.log('2. Or it was saved to a different table/database');
      return;
    }
